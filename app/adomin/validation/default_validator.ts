...
export const DEFAULT_MESSAGE_PROVIDER_CONFIG = {
    ...
    "date.afterOrEqual":
        "Le champ {{ field }} doit être une date après ou avant {{ expectedValue }}",
    "date.beforeOrEqual":
    ...
    "file.extname":
        "Le champ {{ field }} est invalide, les fichiers acceptés sont : {{ extnames }}",
    "file.size": "La champ {{ field }} est invalide, la taille ne doit pas excéder {{ size }}",
    required: "Le champ {{ field }} est requis",
};

export const DEFAULT_MESSAGES_PROVIDER = new SimpleMessagesProvider(
